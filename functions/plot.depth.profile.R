plot.depth.profile <- function (data, location, variable, months=4:10, years=min(data$Year):max(data$Year), add.legend = T) {
  ...
  plot(temp$Result.Value.Text, -temp$SampleDepth, 
      xaxt="n", 
      xlim = c(min(temp$Result.Value.Text), max(temp$Result.Value.Text)), 
      xlab = "", 
      ylab = "Depth", 
      cex.lab = 1.5, 
      bty = "n", 
      col = colours[as.numeric(as.factor(temp$Visit.Start.Date))])
  points(temp2$Median.Value, -temp2$SampleDepth, 
         pch = 19, 
         cex = 2, 
         lwd = 3, 
         type = "o")
