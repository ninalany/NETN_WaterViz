...
shinyServer(function(input,output){
  ...
  #### Fit trend models ----
  #TheilSen no season
  NoSeasonTrend <- reactive({
    if(input$trendType == "Theil-Sen (NOT deseasoned)"){
      if(input$acf == TRUE){
        
        TheilSen(mydata = DataGraph(), pollutant = "value", autocor= TRUE, deseason = FALSE, plot=FALSE)
      }else{
        TheilSen(mydata = DataGraph(), pollutant = "value", autocor= FALSE, deseason = FALSE, plot=FALSE)
      }
    }
  })
  
  # TheilSen seasonal
  SeasonalTrend <-reactive({
    if(input$trendType == "Theil-Sen (deseasoned)"){
      if(input$acf2 == TRUE){
        TheilSen(mydata = DataGraph(), pollutant = "value", autocor= TRUE, deseason = TRUE, plot=FALSE, type="month")
      }else{
        TheilSen(mydata = DataGraph(), pollutant = "value", autocor= FALSE, deseason = TRUE, plot=FALSE, type="month")
      }
    }
  }) 
  
  #Akritas-Theil-Sen (for censored data)
  
  ArkitTheil <-reactive({
    if(input$trendType == "Akritas-Theil-Sen (for censored data)"){    
      #first create vector to indicate which observations are censored:
      
      model_data<-DataGraph() %>%
        mutate(ycen = ifelse(DataGraph()$Result.Value.Text == "*Present <QL", TRUE, FALSE)) %>% 
        ...
        mutate(year.dec = julian(DataGraph()$Visit.Start.Date)/365)
      
      #run analysis
      cenken(y = model_data$value, ycen = model_data$ycen, x = model_data$year.dec)
    }
  })
  ...
  output$plot <- renderPlot({
    ...
    if(input$trendType == "Theil-Sen (NOT deseasoned)"){
      ...
      p <- (p + points(ex, why, lty = line.type, type = "l", lwd = 2, col ="blue"))
    ...
    # calculate trend for censored data
      ...
      p <- (p + points(ex+1970, why, lty = line.type, type = "l", col ="blue"))
  ...
  output$modelout <- renderTable({
    ...
    if(input$trendType == "Theil-Sen (NOT deseasoned)" ){
      ...
      #output text from model fit
      # setup unit labeling in table
      unitsLab<- ifelse(is.na(DataGraph()$Units[1]), "Slope (units/ year)",
                        paste0("Slope (",DataGraph()$Units[1],"/year)"))
      
      slope= ArkitTheil()$slope
      p= ArkitTheil()$p
      int= ArkitTheil()$intercept
      
      data.frame(slope= slope, p= p, intercept = int) %>% 
